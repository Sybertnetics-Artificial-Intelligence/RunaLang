...
class RunaLexer:
    ...
    def _tokenize_internal(self) -> List[Token]:
        """
        ...
        try:
            return self._tokenize_with_error_recovery()
    ...
    def _tokenize_with_error_recovery(self) -> List[Token]:
        """Internal tokenization with error recovery."""
        error_count = 0
        max_errors = 50  # Prevent infinite loops
        ...
        while self.position < len(self.source):
            try:
                ...
                # Unknown character - attempt error recovery
                error_count += 1
                if error_count > max_errors:
                    raise LexerError("Too many lexical errors", self.line, self.column)
                
                # Skip the unknown character and continue
                self.advance()
                continue
                ...
                # Simple error recovery: skip current character and continue
                self.advance()
                continue
    ...
    def tokenize(self, source: str = None) -> List[Token]:
        """Tokenize source code. If source is provided, update internal source."""
