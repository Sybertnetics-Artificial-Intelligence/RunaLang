...
from ..ast.ai import (
    ModelDefinition, ModelStatement, InputLayerStatement, ConvolutionalLayerStatement,
    DownsamplingStatement, ResidualConnectionStatement, OutputLayerStatement,
    TrainingConfig, TrainingStatement, DatasetStatement, AugmentationStatement,
    OptimizerStatement, TrainStatement, SaveModelStatement, KnowledgeQuery
)
...
class BytecodeGenerator:
    ...
    def generate(self, node: Node) -> BytecodeModule:
        """
        Generate bytecode from AST.
        
        Args:
            node: The AST to generate bytecode from
    ...
    def _generate_node(self, node: Node) -> None:
        ...
        elif node.node_type == NodeType.KNOWLEDGE_QUERY:
            self._generate_knowledge_query(node)
        elif node.node_type == NodeType.MODEL_DEFINITION:
            self._generate_model_definition(node)
        elif node.node_type == NodeType.TRAINING_CONFIG:
            self._generate_training_config(node)
        else:
    ...
    def _generate_knowledge_query(self, node: KnowledgeQuery) -> List[Instruction]:
        """Generate bytecode for a knowledge query."""
        instructions = []
        
        # Push the query method name
        method_constant_index = self.add_constant(node.method)
        instructions.append(Instruction(OpCode.LOAD_CONST, method_constant_index))
        
        # Push the arguments
        for arg in node.arguments:
            instructions.extend(self.generate(arg))
        
        # Push the number of arguments
        num_args_constant_index = self.add_constant(len(node.arguments))
        instructions.append(Instruction(OpCode.LOAD_CONST, num_args_constant_index))
        
        # Execute the knowledge graph query
        instructions.append(Instruction(OpCode.KG_QUERY, 0))
        
        return instructions
    
    def _generate_model_definition(self, node: ModelDefinition) -> List[Instruction]:
        """Generate bytecode for a neural network model definition."""
        instructions = []
        
        # Push the model name
        model_name_index = self.add_constant(node.name)
        instructions.append(Instruction(OpCode.LOAD_CONST, model_name_index))
        
        # Process model statements to build a model configuration dictionary
        model_config = {}
        
        for stmt in node.body.statements:
            if isinstance(stmt, InputLayerStatement):
                # Process input layer statement
                instructions.extend(self.generate(stmt.shape_expr))
                instructions.append(Instruction(OpCode.STORE_NAME, self.add_name("input_shape")))
                model_config["input_shape"] = None  # Will be set at runtime
            
            elif isinstance(stmt, ConvolutionalLayerStatement):
                # Process convolutional layer statement
                filters_index = self.add_constant(stmt.filters)
                instructions.append(Instruction(OpCode.LOAD_CONST, filters_index))
                instructions.append(Instruction(OpCode.STORE_NAME, self.add_name("filters")))
                model_config["filters"] = stmt.filters
            
            elif isinstance(stmt, DownsamplingStatement):
                # Process downsampling statement
                true_index = self.add_constant(True)
                instructions.append(Instruction(OpCode.LOAD_CONST, true_index))
                instructions.append(Instruction(OpCode.STORE_NAME, self.add_name("double_filters")))
                model_config["double_filters"] = True
            
            elif isinstance(stmt, ResidualConnectionStatement):
                # Process residual connection statement
                true_index = self.add_constant(True)
                instructions.append(Instruction(OpCode.LOAD_CONST, true_index))
                instructions.append(Instruction(OpCode.STORE_NAME, self.add_name("use_residual")))
                model_config["use_residual"] = True
            
            elif isinstance(stmt, OutputLayerStatement):
                # Process output layer statement
                classes_index = self.add_constant(stmt.classes)
                instructions.append(Instruction(OpCode.LOAD_CONST, classes_index))
                instructions.append(Instruction(OpCode.STORE_NAME, self.add_name("num_classes")))
                
                activation_index = self.add_constant(stmt.activation)
                instructions.append(Instruction(OpCode.LOAD_CONST, activation_index))
                instructions.append(Instruction(OpCode.STORE_NAME, self.add_name("activation")))
                
                model_config["num_classes"] = stmt.classes
                model_config["activation"] = stmt.activation
        
        # Create the model dictionary
        for key, value in model_config.items():
            key_index = self.add_constant(key)
            value_index = self.add_constant(value)
            instructions.append(Instruction(OpCode.LOAD_CONST, key_index))
            instructions.append(Instruction(OpCode.LOAD_CONST, value_index))
        
        dict_size_index = self.add_constant(len(model_config))
        instructions.append(Instruction(OpCode.LOAD_CONST, dict_size_index))
        instructions.append(Instruction(OpCode.BUILD_DICT, 0))
        
        # Call the model creation function (will be resolved at runtime)
        tensorflow_index = self.add_constant("tensorflow")
        instructions.append(Instruction(OpCode.LOAD_CONST, tensorflow_index))
        instructions.append(Instruction(OpCode.LOAD_NAME, self.add_name("create_tensorflow_model")))
        
        # Push the model configuration dictionary as argument
        instructions.append(Instruction(OpCode.ROT_TWO, 0))
        
        # Push the number of arguments (1)
        one_index = self.add_constant(1)
        instructions.append(Instruction(OpCode.LOAD_CONST, one_index))
        
        # Call the function
        instructions.append(Instruction(OpCode.CALL_FUNCTION, 0))
        
        # Store the model in a variable with the given name
        instructions.append(Instruction(OpCode.STORE_NAME, self.add_name(node.name)))
        
        return instructions
    
    def _generate_training_config(self, node: TrainingConfig) -> List[Instruction]:
        """Generate bytecode for a training configuration."""
        instructions = []
        
        # Load the model to train
        instructions.append(Instruction(OpCode.LOAD_NAME, self.add_name(node.model_name)))
        
        # Process training statements to build a training configuration dictionary
        config = {}
        dataset_path = None
        
        for stmt in node.body.statements:
            if isinstance(stmt, DatasetStatement):
                # Process dataset statement
                dataset_path = stmt.path
                dataset_path_index = self.add_constant(dataset_path)
                instructions.append(Instruction(OpCode.LOAD_CONST, dataset_path_index))
                
                # Process dataset configuration expression
                instructions.extend(self.generate(stmt.config_expr))
                instructions.append(Instruction(OpCode.STORE_NAME, self.add_name("dataset_config")))
            
            elif isinstance(stmt, AugmentationStatement):
                # Process augmentation statement
                instructions.extend(self.generate(stmt.augmentation_expr))
                instructions.append(Instruction(OpCode.STORE_NAME, self.add_name("augmentation")))
                config["augmentation"] = True
            
            elif isinstance(stmt, OptimizerStatement):
                # Process optimizer statement
                optimizer_index = self.add_constant(stmt.optimizer_name)
                instructions.append(Instruction(OpCode.LOAD_CONST, optimizer_index))
                instructions.append(Instruction(OpCode.STORE_NAME, self.add_name("optimizer")))
                
                lr_index = self.add_constant(stmt.learning_rate)
                instructions.append(Instruction(OpCode.LOAD_CONST, lr_index))
                instructions.append(Instruction(OpCode.STORE_NAME, self.add_name("learning_rate")))
                
                config["optimizer"] = stmt.optimizer_name
                config["learning_rate"] = stmt.learning_rate
            
            elif isinstance(stmt, TrainStatement):
                # Process train statement
                epochs_index = self.add_constant(stmt.epochs)
                instructions.append(Instruction(OpCode.LOAD_CONST, epochs_index))
                instructions.append(Instruction(OpCode.STORE_NAME, self.add_name("epochs")))
                
                config["epochs"] = stmt.epochs
                
                if stmt.early_stopping_expr:
                    instructions.extend(self.generate(stmt.early_stopping_expr))
                    instructions.append(Instruction(OpCode.STORE_NAME, self.add_name("early_stopping_metric")))
                    config["early_stopping"] = True
            
            elif isinstance(stmt, SaveModelStatement):
                # Process save model statement
                instructions.extend(self.generate(stmt.metric_expr))
                instructions.append(Instruction(OpCode.STORE_NAME, self.add_name("save_metric")))
        ...
        dict_size_index = self.add_constant(len(config))
        instructions.append(Instruction(OpCode.LOAD_CONST, dict_size_index))
        instructions.append(Instruction(OpCode.BUILD_DICT, 0))
        
        # Call the training function (will be resolved at runtime)
        instructions.append(Instruction(OpCode.LOAD_NAME, self.add_name("train_model")))
        
        # Push the model, dataset path, and config as arguments
        # Note: The model is already on the stack
        if dataset_path:
            dataset_path_index = self.add_constant(dataset_path)
            instructions.append(Instruction(OpCode.LOAD_CONST, dataset_path_index))
        else:
            # Default dataset path if none provided
            empty_str_index = self.add_constant("")
            instructions.append(Instruction(OpCode.LOAD_CONST, empty_str_index))
        
        # Push the configuration dictionary
        instructions.append(Instruction(OpCode.ROT_THREE, 0))
        
        # Push the number of arguments (3)
        three_index = self.add_constant(3)
        instructions.append(Instruction(OpCode.LOAD_CONST, three_index))
        
        # Call the function
        instructions.append(Instruction(OpCode.CALL_FUNCTION, 0))
        
        # Store the training result
        result_var_name = f"{node.model_name}_training_result"
        instructions.append(Instruction(OpCode.STORE_NAME, self.add_name(result_var_name)))
