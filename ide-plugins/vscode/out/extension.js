...
function initializeLanguageServer(context) {
    ...
    try {
        ...
        client = new node_1.LanguageClient('runaLanguageServer', 'Runa Language Server', serverOptions, clientOptions);
        outputChannel.appendLine('🚀 Starting Runa Language Server...');
        client.start().then(() => {
            outputChannel.appendLine('✅ Runa Language Server started successfully');
            vscode.window.showInformationMessage('🎉 Runa Language Server is now running! Enjoy intelligent code assistance.', 'Open Output').then(selection => {
...
function showLspNotFoundMessage() {
    vscode.window.showWarningMessage('Runa Language Server not found. Please install it or disable language server features.', 'Install Instructions', 'Disable Language Server', 'Retry').then(selection => {
...
function showLspErrorMessage(error) {
    const errorMsg = error instanceof Error ? error.message : String(error);
    vscode.window.showErrorMessage(`Failed to start Runa Language Server: ${errorMsg}`, 'View Output', 'Disable Language Server').then(selection => {
...
function registerLanguageProviders(context) {
    // Document formatter
    context.subscriptions.push(vscode.languages.registerDocumentFormattingEditProvider('runa', {
        provideDocumentFormattingEdits(document) {
            return formatRunaDocument(document);
        }
    }));
    // Hover provider
    context.subscriptions.push(vscode.languages.registerHoverProvider('runa', {
        provideHover(document, position) {
            return provideRunaHover(document, position);
        }
    }));
    // Completion provider
    context.subscriptions.push(vscode.languages.registerCompletionItemProvider('runa', {
        provideCompletionItems(document, position) {
            return provideRunaCompletions(document, position);
        }
    }, '.', ' ', ':'));
    // Diagnostic provider for mathematical symbol enforcement
    context.subscriptions.push(vscode.languages.registerCodeActionsProvider('runa', {
...
async function compileFile() {
    ...
    try {
        ...
        const process = (0, child_process_1.spawn)(compilerPath, ['compile', filePath], {
        ...
        process.on('error', (error) => {
            ...
            if (error.message.includes('ENOENT')) {
                vscode.window.showErrorMessage('Runa compiler not found. Please check the runa.compiler.path setting.', 'Open Settings').then(selection => {
...
async function runFile() {
    ...
    try {
        // Save the document first
        await activeEditor.document.save();
        const process = (0, child_process_1.spawn)(compilerPath, ['run', filePath], {
            cwd: vscode.workspace.workspaceFolders?.[0]?.uri.fsPath
        });
        process.stdout.on('data', (data) => {
...
function formatRunaDocument(document) {
    const edits = [];
    let indentLevel = 0;
    const indentSize = 4;
    for (let i = 0; i < document.lineCount; i++) {
...
function provideRunaHover(document, position) {
...
function provideRunaCompletions(document, position) {
    const completions = [];
    const config = vscode.workspace.getConfiguration('runa');
...
function provideMathSymbolFixes(document, range) {
    const actions = [];
    const config = vscode.workspace.getConfiguration('runa');
    ...
    if (matches) {
        matches.forEach(symbol => {
            const action = new vscode.CodeAction(`Convert '${symbol}' to natural language`, vscode.CodeActionKind.QuickFix);
            action.command = {
