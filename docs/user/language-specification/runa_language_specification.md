...
| Access Type | What | Canon | Developer |
|-------------|------|-------|-----------|
...
│ - Allocation: 0 ns (compiler-managed)                          │
│ - Deallocation: 0 ns (automatic at scope end)                  │
│ - Overhead: 0 bytes                                            │
│ - Use case: Loop counters, temporary calculations              │
│ - Real-time safe: ✅ YES (deterministic)                       │
├────────────────────────────────────────────────────────────────┤
│ TIER 2: ARENA                                                  │
│ - Bulk temporary allocations, operation-scoped                 │
│ - Allocation: ~10 ns (pointer bump)                            │
│ - Deallocation: ~100 ns (O(1) bulk free)                       │
│ - Overhead: ~24 bytes per arena + unused capacity              │
│ - Use case: Compiler temps, parsing, batch processing          │
│ - Real-time safe: ✅ YES (deterministic bulk cleanup)          │
├────────────────────────────────────────────────────────────────┤
│ TIER 3: OWNED                                                  │
│ - Individual heap allocations, structured ownership            │
│ - Allocation: ~100 ns (malloc)                                 │
│ - Deallocation: ~50 ns (free, called by destructor)            │
│ - Overhead: ~16-32 bytes per allocation                        │
│ - Use case: Program AST, permanent data structures             │
│ - Real-time safe: ✅ YES (deterministic, scope-based)          │
├────────────────────────────────────────────────────────────────┤
│ TIER 4: SHARED (Manual Reference Counting)                     │
│ - Explicit reference counting for multiple owners              │
│ - Allocation: ~100 ns (malloc + refcount init)                 │
│ - Retain: ~5 ns (atomic increment - manual)                    │
│ - Release: ~5-50 ns (atomic decrement + maybe free - manual)   │
│ - Overhead: ~24-40 bytes (malloc + refcount)                   │
│ - Use case: When you need explicit control over sharing        │
│ - Real-time safe: ✅ YES (deterministic, manual control)       │
├────────────────────────────────────────────────────────────────┤
│ TIER 5: ARC (Automatic Reference Counting)                     │
│ - Compiler-managed reference counting (like Swift)             │
│ - Allocation: ~100 ns (malloc + refcount init)                 │
│ - Retain: ~5 ns (atomic increment - compiler-inserted)         │
│ - Release: ~5-50 ns (atomic decrement + maybe free - compiler) │
│ - Overhead: ~24-40 bytes (malloc + refcount)                   │
│ - Use case: Convenience for non-critical code paths            │
│ - Real-time safe: ✅ YES (deterministic, immediate cleanup)    │
│ - Cycle handling: weak references (programmer-specified)       │
└────────────────────────────────────────────────────────────────┘
```

#### Tier Comparison

| Tier | Speed | Ease | Control | Real-Time | Use When |
|------|-------|------|---------|-----------|----------|
| 1: Stack | Fastest (0ns) | Automatic | None needed | ✅ YES | Fixed-size locals |
| 2: Arena | Very Fast (10ns) | Semi-auto | Arena lifetime | ✅ YES | Temporary bulk data |
| 3: Owned | Fast (100ns) | Auto* | Ownership | ✅ YES | Permanent structures |
| 4: Shared | Fast (100ns+5ns) | Manual | Full control | ✅ YES | Explicit sharing |
| 5: ARC | Fast (100ns+5ns) | Automatic | Minimal | ✅ YES | Convenient sharing |

*With constructors/destructors (future)

#### Memory Allocation Details

**Stack Allocation (Tier 1):**
- Compiler allocates at function entry
- Zero runtime overhead
- Limited to ~1-8MB total stack size
- Automatically freed at scope end

**Arena Allocation (Tier 2):**
- Bump pointer allocation (sequential)
- All allocations freed in O(1) when arena destroyed
- Typical usage: Create arena → allocate temps → destroy arena
- Ideal for operations with clear start/end (compilation, request handling)

**Owned Allocation (Tier 3):**
- Standard malloc/free with ownership semantics
- Compiler tracks ownership (future: Rust-style borrow checking)
- Destructor automatically called at scope end (future)
- Current: Manual via allocate()/deallocate()

**Shared Manual (Tier 4):**
- Programmer explicitly calls retain/release
- Full control over reference counting
- Use when you need to reason about exact timing
- Atomic operations for thread safety

**ARC (Tier 5):**
- Compiler automatically inserts retain/release calls
- Same performance as Tier 4, zero manual effort
- Handles most "convenience" use cases
- Requires weak references to break cycles

#### Cycle Handling in ARC

```runa
Note: ARC with cycle prevention
Type called "Node" with annotation @ARC:
    value as Integer
    next as Node           Note: Strong reference (increments refcount)
...
Note: When a and b go out of scope:
Note: b.count → 0, freed immediately
Note: a.count → 0, freed immediately
Note: No memory leak! ✅
```
...
- Runa does NOT use tracing garbage collection
- All memory management is deterministic
- No stop-the-world pauses
- No unpredictable collection cycles
- Safe for real-time, embedded, and mission-critical systems

**Real-Time Guarantee:**
- All 5 tiers have predictable, bounded execution time
- Stack: 0ns (compiler-managed)
- Arena: 10ns allocation, 100ns bulk free
- Owned: 100ns allocation, 50ns free
- Shared: 5ns retain/release
- ARC: 5ns retain/release (compiler-inserted)
...
ARC                     | ~110 ms   | 32MB + 24-40MB overhead
```
