...
// File I/O functions for import system
...
int64_t read_file_data(FILE* file, void* buffer, int64_t size) {
    if (!file || !buffer) return 0;
    return fread(buffer, 1, size, file);
}

int64_t get_file_size_internal(FILE* file) {
    if (!file) return 0;
...
int64_t stat_file(const char* filename) {
    struct stat st;
...
int64_t get_mtime_from_stat(int64_t stat_ptr) {
    struct stat* st = (struct stat*)stat_ptr;
    return st->st_mtime;
}
