...
static void program_add_global(Program *program, GlobalVariable *global) {
    if (program->global_count >= program->global_capacity) {
        program->global_capacity = program->global_capacity == 0 ? 4 : program->global_capacity * 2;
        program->globals = realloc(program->globals, sizeof(GlobalVariable*) * program->global_capacity);
    }
    program->globals[program->global_count++] = global;
}
...
static Expression* parser_parse_expression(Parser *parser) {
    ...
    while (parser->current_token->type == TOKEN_DOT || parser->current_token->type == TOKEN_LBRACKET) {
        if (parser->current_token->type == TOKEN_DOT) {
            ...
            Expression *array_index = malloc(sizeof(Expression));
            array_index->type = EXPR_ARRAY_INDEX;
            array_index->data.array_index.array = left;
            array_index->data.array_index.index = index;
            left = array_index;
        }
    }
    ...
    while (parser->current_token->type == TOKEN_PLUS || parser->current_token->type == TOKEN_MINUS ||
        ...
        // Handle field access and array indexing on the right operand
        while (parser->current_token->type == TOKEN_DOT || parser->current_token->type == TOKEN_LBRACKET) {
            if (parser->current_token->type == TOKEN_DOT) {
                parser_eat(parser, TOKEN_DOT);
                ...
                Expression *array_index = malloc(sizeof(Expression));
                array_index->type = EXPR_ARRAY_INDEX;
                array_index->data.array_index.array = right;
                array_index->data.array_index.index = index;
                right = array_index;
            }
        }
...
static Statement* parser_parse_match_statement(Parser *parser) {
    ...
    while (parser->current_token->type == TOKEN_WHEN) {
        ...
        if (parser->current_token->type == TOKEN_WITH) {
            ...
            // Parse field bindings: field1 as var1 and field2 as var2...
            int field_capacity = 10;
            field_names = malloc(sizeof(char*) * field_capacity);
    ...
    // Check for variant syntax: Type Name is | Variant1 ...
        ...
        if (parser->current_token->type == TOKEN_COLON) {
            ...
            if (parser->current_token->type == TOKEN_ARRAY) {
                parser_eat(parser, TOKEN_ARRAY);
                parser_eat(parser, TOKEN_LBRACKET);

                // Parse array size
                if (parser->current_token->type != TOKEN_INTEGER) {
                    fprintf(stderr, "[PARSER ERROR] Expected array size at line %d\n", parser->current_token->line);
                    exit(1);
                }
                int array_length = atoi(parser->current_token->value);
                parser_eat(parser, TOKEN_INTEGER);
                parser_eat(parser, TOKEN_RBRACKET);
                parser_eat(parser, TOKEN_OF);

                // Parse element type
                char *element_type = NULL;
                if (parser->current_token->type == TOKEN_INTEGER_TYPE) {
                    element_type = string_duplicate("Integer");
                    parser_eat(parser, TOKEN_INTEGER_TYPE);
                } else if (parser->current_token->type == TOKEN_STRING_TYPE) {
                    element_type = string_duplicate("String");
                    parser_eat(parser, TOKEN_STRING_TYPE);
                } else if (parser->current_token->type == TOKEN_CHARACTER_TYPE) {
                    element_type = string_duplicate("Character");
                    parser_eat(parser, TOKEN_CHARACTER_TYPE);
                } else if (parser->current_token->type == TOKEN_IDENTIFIER) {
                    element_type = string_duplicate(parser->current_token->value);
                    parser_eat(parser, TOKEN_IDENTIFIER);
                } else {
                    fprintf(stderr, "[PARSER ERROR] Expected element type at line %d\n", parser->current_token->line);
                    exit(1);
                }

                // Setup array type
                type->kind = TYPE_KIND_ARRAY;
                type->data.array_type.element_type = element_type;
                type->data.array_type.length = array_length;

                // Calculate element size (8 bytes for Integer, custom types handled separately)
                type->data.array_type.element_size = 8;
                type->size = array_length * type->data.array_type.element_size;

                // Parse "End Type" after array type definition
                parser_eat(parser, TOKEN_END);
                parser_eat(parser, TOKEN_TYPE);
            } else {
                fprintf(stderr, "[PARSER ERROR] Expected array or variant declaration after colon at line %d\n",
                        parser->current_token->line);
                exit(1);
            }
        }
        // Check for function pointer type: Type X is Pointer to Process ...
        else if (parser->current_token->type == TOKEN_POINTER) {
            parser_eat(parser, TOKEN_POINTER);
            parser_eat(parser, TOKEN_TO);
            parser_eat(parser, TOKEN_PROCESS);

            // This is a function pointer type
            type->kind = TYPE_KIND_FUNCTION;
            type->data.function_type.param_types = NULL;
            type->data.function_type.param_count = 0;
            type->data.function_type.return_type = NULL;
            type->size = 8;  // Pointer size on x86_64
            ...
            } else if (parser->current_token->type == TOKEN_TAKES) {
                parser_eat(parser, TOKEN_TAKES);
            }
            ...
            if (parser->current_token->type == TOKEN_IDENTIFIER) {
                ...
                type->data.function_type.param_types = malloc(sizeof(char*) * param_capacity);
                ...
                do {
                    ...
                    if (parser->current_token->type == TOKEN_INTEGER_TYPE) {
                        param_type = string_duplicate("Integer");
                        parser_eat(parser, TOKEN_INTEGER_TYPE);
                    } else if (parser->current_token->type == TOKEN_STRING_TYPE) {
                        param_type = string_duplicate("String");
                        parser_eat(parser, TOKEN_STRING_TYPE);
                    } else if (parser->current_token->type == TOKEN_CHARACTER_TYPE) {
                        param_type = string_duplicate("Character");
                        parser_eat(parser, TOKEN_CHARACTER_TYPE);
                    } else if (parser->current_token->type == TOKEN_IDENTIFIER) {
                    ...
                    if (type->data.function_type.param_count >= param_capacity) {
                        param_capacity *= 2;
                        type->data.function_type.param_types = realloc(
                            type->data.function_type.param_types,
                            sizeof(char*) * param_capacity
                        );
                    }

                    type->data.function_type.param_types[type->data.function_type.param_count++] = param_type;
            ...
            if (parser->current_token->type == TOKEN_INTEGER_TYPE) {
                type->data.function_type.return_type = string_duplicate("Integer");
                parser_eat(parser, TOKEN_INTEGER_TYPE);
            } else if (parser->current_token->type == TOKEN_STRING_TYPE) {
                type->data.function_type.return_type = string_duplicate("String");
                parser_eat(parser, TOKEN_STRING_TYPE);
            } else if (parser->current_token->type == TOKEN_CHARACTER_TYPE) {
                type->data.function_type.return_type = string_duplicate("Character");
                parser_eat(parser, TOKEN_CHARACTER_TYPE);
            } else if (parser->current_token->type == TOKEN_IDENTIFIER) {
                type->data.function_type.return_type = string_duplicate(parser->current_token->value);
                parser_eat(parser, TOKEN_IDENTIFIER);
            } else {
        ...
        }  // End of variant type else block
    } else {
...
Program* parser_parse_program(Parser *parser) {
    ...
    while (parser->current_token->type != TOKEN_EOF) {
        if (parser->current_token->type == TOKEN_IMPORT) {
            ...
            Expression *initial_value = parser_parse_expression(parser);

            // Create global variable
            GlobalVariable *global = malloc(sizeof(GlobalVariable));
            global->name = var_name;
            global->type = string_duplicate("Integer");  // For now, all globals are integers
...
void program_destroy(Program *program) {
    if (program) {
        ...
        for (int i = 0; i < program->global_count; i++) {
            ...
            free(program->globals[i]->type);
            expression_destroy(program->globals[i]->initial_value);
            free(program->globals[i]);
        }
