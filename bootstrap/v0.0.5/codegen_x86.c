...
static void codegen_generate_expression(CodeGenerator *codegen, Expression *expr) {
    switch (expr->type) {
        ...
        case EXPR_FUNCTION_CALL: {
            ...
            // For now, we support up to 2 arguments (sufficient for list functions)
            if (expr->data.function_call.argument_count > 0) {
                ...
                if (expr->data.function_call.argument_count > 1) {
                    // Save first argument
                    fprintf(codegen->output_file, "    pushq %%rdi\n");

                    // Second argument
                    codegen_generate_expression(codegen, expr->data.function_call.arguments[1]);
                    fprintf(codegen->output_file, "    movq %%rax, %%rsi\n");

                    // Restore first argument
                    fprintf(codegen->output_file, "    popq %%rdi\n");
                }
            }
