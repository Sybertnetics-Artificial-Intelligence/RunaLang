; ModuleID = 'test_v02_assembly_concept.s'
source_filename = "test_v02_assembly_concept.s"
...
entry:
  ...
  %x.load = load i64, ptr %x, align 4
  %y.load = load i64, ptr %y, align 4
  %add = add i64 %x.load, %y.load
  %result = alloca i64, align 8
  store i64 %add, ptr %result, align 4
  %result.load = load i64, ptr %result, align 4
  %call = call i64 @test_function(i64 %result.load)
  %call_result = alloca i64, align 8
  store i64 %call, ptr %call_result, align 4
  %call_result.load = load i64, ptr %call_result, align 4
  ret i64 %call_result.load
}

define i64 @test_function(i64 %val) {
entry:
  %val1 = alloca i64, align 8
  store i64 %val, ptr %val1, align 4
  %val.load = load i64, ptr %val1, align 4
  %add = add i64 %val.load, 1
  ret i64 %add
}
